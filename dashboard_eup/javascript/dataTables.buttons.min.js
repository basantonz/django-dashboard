...
(function (e) {
        ...
        _constructor: function () {
            ...
            e("body").on("keyup." + this.s.namespace, function (l) {
                if (!y.activeElement || y.activeElement === y.body) {
                    var k = String.fromCharCode(l.keyCode).toLowerCase(); - 1 !== a.s.listenKeys.toLowerCase().indexOf(k) && a._keypress(k, l)
                }
            })
        },
        _addKey: function (a) {
    ...
    v.buttonSelector = function (a, b) {
        for (var c = [], d = function (k, n, p) {
                ...
                if (Array.isArray(k) || k instanceof e)
                    ...
                    for (f = 0, p = r.length; f < p; f++) c.push({
                        inst: n,
                        node: r[f].node
                    });
                else if ("number" === typeof k) c.push({
                    inst: n,
                    node: n.s.buttons[k].node
                });
                else if ("string" ===
                    typeof k)
                    if (-1 !== k.indexOf(","))
                        for (r = k.split(","), f = 0, p = r.length; f < p; f++) h(r[f].trim(), n);
                    else if (k.match(/^\d+(\-\d+)*$/)) f = e.map(r, function (m) {
                    return m.idx
                }), c.push({
                    inst: n,
                    ...
                    for (k = k.replace(":name", ""), f = 0, p = r.length; f < p; f++) r[f].name === k && c.push({
                        inst: n,
                        node: r[f].node
                    });
    ...
    e.extend(z, {
        ...
        pageLength: function (a) {
            ...
            return {
                extend: "collection",
                text: function (g) {
                    return g.i18n("buttons.pageLength", {
                        "-1": "Show all rows",
                        _: "Show %d rows"
                    }, g.page.len())
                },
    ...
    var K = function (a) {
        ...
        Q = function (a, b) {
            ...
            a.select && "function" === typeof a.select.info && h.selected === q && a.rows(c.rows, e.extend({
                selected: !0
            }, h)).any() && e.extend(h, {
                selected: !0
            });
